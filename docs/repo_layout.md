...
| `data/` | canonical machine-readable inputs the seeds read: `decay_energies.csv`, `pixel_wiring.csv`, `source_installations.csv`, `sources` spreadsheet, `bc_maps.csv`; `data/TrapFilterData/` is the cluster-side working area for trap filter jobs |
| `data/provenance/` | the human documents the seeded data came from: the Source Installation History PDF, the corrected 7/21 installation figure, the pixel preamp/FET map figures |
