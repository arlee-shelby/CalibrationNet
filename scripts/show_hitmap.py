...
energy arrays themselves are not pulled).
    ...
    python scripts/show_hitmap.py 8622 --det upper --out hitmaps/
"""
...
from calibrationnet.models import Pixel, Run, RunPixel, TrapFilterOutput


def fetch_counts(run_number: int, detector: str, label: str) -> dict:
    """{pixel_number: waveform count} for one run/detector."""
    with get_session() as session:
        rows = session.execute(
            ...
            .join(RunPixel.pixel)
            .join(RunPixel.run)
            .where(Run.run_number == run_number,
...
def draw(run_number: int, detector: str, counts: dict, out_dir: Path,
         vmax: float = None) -> Path:
    ...
    ax.set_aspect("equal")
    ax.axis("off")
    ax.set_title(f"Run {run_number} — {detector} detector "
                 f"(waveforms per pixel, scale capped at {int(vmax)})")
    sm = plt.cm.ScalarMappable(cmap=cmap, norm=plt.Normalize(0, vmax))
    fig.colorbar(sm, ax=ax, shrink=0.8)

    out_dir.mkdir(parents=True, exist_ok=True)
    out = out_dir / f"Run{run_number}_{detector}_hitmap.png"
...
def main() -> None:
    ...
    counts = fetch_counts(args.run_number, args.det, args.label)
    if not counts:
        raise SystemExit(f"No '{args.label}' filter outputs stored for run "
                         f"{args.run_number} ({args.det}).")
    out = draw(args.run_number, args.det, counts, args.out, args.vmax)
